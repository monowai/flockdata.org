...
curl -u mike:123 -H "Content-Type:application/json" -X POST http://localhost:8080/v1/profiles/ -d '{"name":"mike", "companyName":"Monowai","password":"whocares"}'
...
# curl -u mike:123 -H "Content-Type:application/json" -X POST http://localhost:8080/v1/fortress/ -d '{"name": "law-suits","searchActive": true}'

curl -u mike:123 -H "Content-Type:application/json" -X PUT http://localhost:8080/v1/tag/ --data-binary @./suits.json
