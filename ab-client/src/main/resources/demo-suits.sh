...
## Here, law-suits is the name of our fortress. Fortress is only required if you are going to load audit records
# curl -u mike:123 -H "Content-Type:application/json" -X POST http://localhost:8080/ab-engine/v1/fortress/ -d '{"name": "law-suits","searchActive": true}'

curl -u mike:123 -H "Content-Type:application/json" -X PUT http://localhost:8080/ab-engine/v1/tag/ --data-binary @./suits.json
