...
public class AccountServiceTest {
    ...
    public void testSaveAccount() throws IllegalAccessException, IOException {
        String accountKey = new DateTime().toString();
        Account account = new Account();
        account.setAccountNumber(accountKey);
        account.setIban("iban");
        account.setStatus("CREATED");

        Long accountId = accountService.saveAccount(account).getId();

        Account accountDb = accountRepository.findOne(accountId);
        Assert.assertNotNull(accountDb);
        Assert.assertEquals(accountDb.getAccountNumber(), accountKey);
