...
FRIENDLY_NAME="AuditBucket Engine Service"
...
checkstatus() {
  ...
  if [ -e "$PID_FILE" ] ; then
    ABE_PID=$( cat "$PID_FILE" )
    kill -0 $ABE_PID 2>/dev/null || ABE_PID=
  fi
}
...
detectrunning() {
  ...
  newpid=$(lsof -i :$ABE_HTTP_PORT -F T -Ts | grep -i "TST=LISTEN" -B1 | head -n1)
  newpid=${newpid:1}
}
     ...
     if [ $DIST_OS = "macosx" ] ; then
      ...
      if [ -z $ABE_PID ] ; then
        ...
        $JAVA -cp '$CLASSPATH' -jar $AB_APP/ab-engine-1.0.0-BUILD-SNAPSHOT-war-exec.jar $JAVA_OPTS  \
            ...
            -DAB_HOME=${AB_HOME} \
            -Dlog4j.configuration=file://${AB_CONFIG}/log4j-engine.xml& > "$AB_HOME/log/abe-console.txt" & ps aux |grep [h]ttpPort=${ABE_HTTP_PORT} | awk '/[a]b-engine/{print $2}' >> $PID_FILE
...
stopit() {
  ...
  if [ -z $ABE_PID ] ; then
    ...
    printf "Stopping $FRIENDLY_NAME [$ABE_PID]..."
    x=0
    while [ $x -lt $TIMEOUT ] && [ "$ABE_PID" != "" ]  ; do
      kill $ABE_PID 2>/dev/null
      printf "."
      sleep 1
      detectrunning
      if [ !"$newpid" ] ; then
